...
モンスター選択 (pick_monster)
  ↓
ラウンド 1〜N (N = state.totalRounds, 1-3, デフォルト3)
  └── ミニラウンド 1〜M (M = state.miniRoundsPerRound, 1-3, デフォルト3)
        └── イベント → アクション → ドラフト
              ↓ (ミニラウンドM完了)
            バトル → 鍛え直し (最終ラウンド以外)
  ↓ (最終ラウンド完了)
...
ラウンド数とミニラウンド数はルーム作成時にホストが 1〜3 のスライダーで設定可能。
...
- 全員が同時に1体を選択（ドラフト形式）
- **被り解消ルール：**
  - 唯一選んだプレイヤー → そのモンスター獲得（プールから除去）
  - 複数が同じモンスターを選択 → 全員が再選択（被ったカードはプールに残る）
  - 最大8回試行；収束しなければフォールバック分配（ランダム）
...
- **手札制：** 各プレイヤーは個人の手札（`Player.actionHand`）を持つ
  - **ゲーム開始時**（`createInitialState` の時点 = モンスター選択前）に共通山札から **3枚** ドロー
  - 各アクションフェーズ開始時に **1枚** ドロー → 手札から1枚選んで使用 → 墓地へ
  - つまり手札サイズは 3↔4 を行き来する
...
### SPD回避（フィーチャーフラグ `SPD_DODGE_ENABLED`、`battle.ts`）

- 攻撃時、防御側のSPDが攻撃側を上回っているとMISS確率が発生
  - 差 1 → 10% / 差 ≥ 10 → 50% / 差 2-9 は線形補間
...
|------|----|-----|-----|-----|-------------|
| デーモン | 18 | 6 | 6 | 2 | 悪の目醒め（初撃ダメージ2倍） |
| ユニコーン | 16 | 4 | 5 | 4 | 清廉潔白（turn_start_heal +1） |
| ゴーレム | 20 | 5 | 7 | 2 | 岩鎧（damage_reduction -1） |
| フェンリル | 10 | 5 | 3 | 8 | 疾風の祝福（spd_roll_bonus +2） |
| エルフ | 10 | 5 | 3 | 6 | 軽やかな舞踊（回避率+15%） |
| ワイバーン | 8 | 6 | 3 | 7 | 急襲の本能（初撃がtrue_damage） |
| スライム | 14 | 5 | 5 | 5 | 形なき適応（battle_start, ATK/DEFの高い方を+2） |
| ウロボロス | 10 | 3 | 4 | 6 | 時の輪廻（atk_per_active +1：攻撃ごとにATK+1） |
| ヴァンパイア | 8 | 6 | 3 | 5 | 吸血（与ダメージの1/2を回復） |
| フェニックス | 12 | 5 | 4 | 5 | 不死鳥の加護（一度だけ最大HP1/4で耐える） |
| バーサーカー | 8 | 7 | 2 | 5 | 怒りの咆哮（被ダメ時にATK+1累積） |
| ウィザード | 12 | 3 | 5 | 5 | 装甲呪詛（与ダメ時に相手DEF-1累積） |
| ケルベロス | 12 | 5 | 4 | 5 | 連携攻撃（攻撃時10%で2回発動） |
| カーバンクル | 14 | 4 | 5 | 4 | 宝玉の反射（被ダメ1/3を相手に反射） |
| ナイト | 14 | 5 | 5 | 3 | 背水の覚悟（HP半分以下でATK+4） |
| バグ | 10 | 5 | 5 | 5 | 崩壊する身体（自分のアクティブ発動後にHP-3, ATK/DEF/SPD-2） |
