...
export function describeActiveEffect(e: SkillEffect): string {
  switch (e.kind) {
    ...
    case 'deja_vu_attack':
      return `${e.useStat.toUpperCase()}×N で攻撃（N=このカードのバトル中の使用回数${e.mult === 1 ? '' : `、係数 ${e.mult}`}）`;
    case 'drain_hp':
...
export function describePassiveEffect(e: PassiveEffect): string {
  switch (e.kind) {
    ...
    case 'paralyze_on_active':
      return `攻撃後${e.percent}%で相手に麻痺[1]付与`;
    ...
    case 'slow_burn':
      return `瘴気[${e.amount}]（自分のターン開始時に相手HP-${e.amount}・DEF無視）`;
    case 'revenge_burst':
