      ...
      >
        ...
        <Section title="用語集">
          <Table
            headers={['用語', '意味']}
            rows={[
              ['廃棄', 'そのスキルはバトル中二度と使用できない（バトル終了後は通常通り扱う）'],
              ['罪', '使うと自分に不利な効果が発生するスキルにつくタグ。保有数に応じてバトル開始時にボーナス：4枚でATK+4、5枚でATK+10/DEF+5、6枚でATK+18/DEF+12/SPD+6、7枚以上でATK+28/DEF+21/SPD+14。さらに5枚以上になると罪スキル自体がATK×1.5（5枚）/×2.5（6枚）/×5.0（7枚以上）の攻撃に変わる'],
              ['瘴気[N]', '自分のターン開始時に相手のHPを N 減らす（DEF・シールド無視）'],
              ['シールド[N]', '次の被ダメージを N 軽減（累計 N まで吸収するとシールド消失）'],
              ['反射シールド[N]', 'シールドと同じ効果。さらに吸収したダメージ量だけ相手にDEF無視で反射'],
              ['幽霊シールド[N]', 'シールドと同じ効果。さらに「吸収量＋相手ATK×0.5」を相手にDEF無視で反射'],
              ['オーラ[N]', 'N-1 以下のダメージを完全無効化。N 以上のダメージを受けると砕け散る'],
            ]}
          />
        </Section>
        ...
        <Section title="モンスター一覧">
          <Table
            ...
            rows={[
              ...
              ['マジシャン', '12', '3', '5', '5', '装甲呪詛：与ダメ時相手DEF-1累積'],
