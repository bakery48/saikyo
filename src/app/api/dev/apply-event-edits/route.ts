...
const OVERRIDES_PATH = path.join(process.cwd(), 'src/server/engine/cards/events.overrides.json');
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const body = await req.json() as Array<{ id: string } & EventOverride>;

  let current: Record<string, EventOverride> = {};
  try {
    current = JSON.parse(fs.readFileSync(OVERRIDES_PATH, 'utf-8')) as Record<string, EventOverride>;
  } catch {
